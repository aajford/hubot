...
#   hubot newrelic web apps - Returns statistics for all applications from New Relic
#   hubot newrelic web apps errors - Returns statistics for applications with errors from New Relic
#   hubot newrelic web apps name <filter_string> - Returns a filtered list of applications
#   hubot newrelic web apps instances <app_id> - Returns a list of one application's instances
#   hubot newrelic web apps hosts <app_id> - Returns a list of one application's hosts
...
#   hubot newrelic android crashes - Returns the crash rate and crash number for the Sky News Android app since midnight
#   hubot newrelic iphone crashes - Returns the crash rate and crash number for the Sky News iPhone app since midnight
#
# Authors:
#   statianzo
#
...
plugin = (robot) ->
  ...
  # midnight = yyyy + '-' + mm + '-' + dd + 'T00:00:01+00:00' 
  # now = yyyy + '-' + mm + '-' + dd + 'T' + hour + ':' + minute + ':' + second + '+00:00' 

  apiBaseUrlMobile = "https://api.newrelic.com/v2/mobile_applications/"
  ...
  requestMobile = (path, data, cb) ->
    robot.http(apiBaseUrlMobile + path)
      .header('X-Api-Key', apiKey)
      .header("Content-Type","application/x-www-form-urlencoded")
      .post(data) (err, res, body) ->
        if err
...
#{robot.name} newrelic web apps\n
#{robot.name} newrelic web apps errors\n
#{robot.name} newrelic web apps name <filter_string>\n
#{robot.name} newrelic web apps instances <app_id>\n
#{robot.name} newrelic web apps hosts <app_id>\n
...
#{robot.name} newrelic iphone crashes\n
#{robot.name} newrelic android crashes"


  robot.respond /(newrelic|nr) web apps$/i, (msg) ->
  ...
  robot.respond /(newrelic|nr) web apps errors$/i, (msg) ->
    request 'applications.json', '', (err, json) ->
      if err
        msg.send "Failed: #{err.message}"
      else
  ...
  robot.respond /(newrelic|nr) web apps name ([\s\S]+)$/i, (msg) ->
  ...
  robot.respond /(newrelic|nr) web apps hosts ([0-9]+)$/i, (msg) ->
  ...
  robot.respond /(newrelic|nr) web apps instances ([0-9]+)$/i, (msg) ->
  ...
  robot.respond /(newrelic|nr) android crashes$/i, (msg) ->
    # data = 'from = ' +  encodeURIComponent(midnight) + '&to=' + encodeURIComponent(now) 
    requestMobile "9176726.json", '', (err, json) ->
      if err
        msg.send "Failed: #{err.message}"
      else
        msg.send "Android (prod) app, in the past 30 minutes...\nCrashes: #{json.application.crash_summary.crash_count}\nCrash Rate: #{json.application.crash_summary.crash_rate}"

  robot.respond /(newrelic|nr) iphone crashes$/i, (msg) ->
    # data = 'from=' +  encodeURIComponent(midnight) + '&to=' + encodeURIComponent(now)
    requestMobile "9550949.json", data, (err, json) ->
      if err
        msg.send "Failed: #{err.message}"
      else
        msg.send data + '\n' + apiBaseUrlMobile + '9550949.json\n'
        msg.send json + '\n'
        msg.send "iphone (prod) app, in the past 30 minutes...\nCrashes: #{json.application.crash_summary.crash_count}\nCrash Rate: #{json.application.crash_summary.crash_rate}"
