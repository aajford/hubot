...
#   hubot newrelic android crashes - Returns the crash rate and crash number for the Sky News Android app in the past 30 minutes
#   hubot newrelic iphone crashes - Returns the crash rate and crash number for the Sky News iPhone app in the past 30 minutes
#
# Authors:
#   statianzo
#
...
#{robot.name} newrelic android crashes"
  ...
  robot.respond /(newrelic|nr) iphone crashes$/i, (msg) ->
    # data = 'from=' +  encodeURIComponent(midnight) + '&to=' + encodeURIComponent(now)
    requestMobile "9550949.json", '', (err, json) ->
      if err
        msg.send "Failed: #{err.message}"
      else
